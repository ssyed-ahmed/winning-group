...
export const _shoppingCartReducer = createReducer(
  initialState,
    ...
    if (foundItem) {
      ...
      if (quantity > 1) {
        ...
        return {
          ...state,
          items: [
            ...existingItems.filter((item) => item.product.sku !== product.sku),
          ],
        };
      }
    }
...
export function shoppingCartReducer(
  ...
  enableES5();
  return produce(state, (draft) => _shoppingCartReducer(draft, action));
}
